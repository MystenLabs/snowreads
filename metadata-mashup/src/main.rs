...
async fn main() -> Result<()> {
    ...
    for (i, batch) in arxiv_batches.into_iter().enumerate() {
        ...
        let arxiv_jsons =
            match fetch_arxiv_api(batch.as_slice(), retry_attempts, retry_delay_ms).await {
                Ok(jsons) => jsons,
                Err(e) => {
                    tracing::error!(
                        "Error fetching arXiv metadata for arXiv IDs {:?}: {}",
                        batch,
                        e
                    );
                    batch
        ...
        let datacite_jsons =
            match fetch_datacite_api(batch.as_slice(), retry_attempts, retry_delay_ms).await {
                Ok(jsons) => jsons,
                Err(e) => {
                    tracing::error!(
                        "Error fetching DataCite metadata for arXiv IDs {:?}: {}",
                        batch,
                        e
                    );
                    batch
